...
export const changelog: ChangelogEntry[] = [
  {
    ...
    date: '2026-03-26',
    added: [
      '🎞️ 私人影库（My Library）: 新增后台配置、连通性测试、媒体扫描、状态/进度查询与独立页面',
      '🔌 多连接器接入: 支持 OpenList、Emby、Jellyfin 等私人媒体源，并补充小雅 Alist 接入说明',
      '⏭️ 跳过预设管理: 支持预设的创建、编辑、删除、导入、导出与冲突预览，可在播放页直接应用',
      '🧾 TMDB 元数据增强: 新增 TMDB 接口与播放页补充信息面板，完善详情、海报和演职员展示',
      '🎧 多音轨能力增强: 支持私有媒体流与 HLS 音轨解析、切换与状态重置',
      '🧭 来源与详情补强: 新增私人影库海报代理、来源命名规范化、Douban ID 解析与更多元数据补充能力',
    ],
    changed: [
      '🛡️ 搜索与列表稳健性升级: 强化过滤、排序与 WebSocket 搜索链路的数据防御，缺失字段时也能稳定展示',
      '🖼️ 图片加载链路升级: 引入统一外部图片组件、占位图、解码提示、海报优先级控制与异常降级',
      '⚡ 虚拟化列表体验优化: 调整虚拟化阈值、超扫描、数据安全校验与源浏览器加载更多逻辑',
      '🧭 顶部导航体验优化: 重构导航项渲染、滚动反馈与溢出处理逻辑，增强局部状态管理',
      '🎮 播放交互优化: 持久化播放倍速与弹幕偏好，增强移动端拖动进度预览反馈与播放配置一致性',
      '🧩 配置与数据源管理优化: 重构配置持久化、配置校验与视图刷新流程，增强 CMS 代理、成人源过滤与分类处理能力',
      '📚 My Library 浏览能力增强: 支持私人影库条目的搜索、分类筛选、分页、初始状态管理、缓存预处理与播放记录联动',
    ],
    fixed: [
      '🐛 修复搜索结果在标题、年份、集数缺失时的排序、过滤、比较与展示异常',
      '🐛 修复图片加载失败、无效数据与组件异常导致的列表崩溃问题，并补充错误边界兜底',
      '🐛 修复注册空密码、配置保存校验、导入格式验证和配置变更后视图不同步等问题',
      '🐛 修复预设导入反馈不足、占位文案不清晰与部分播放配置持久化不一致的问题',
      '🐛 修复 HLS 音轨切换、音轨状态残留与私人媒体播放细节中的若干稳定性问题',
    ],
  },
  {
    version: '1.3.0',
